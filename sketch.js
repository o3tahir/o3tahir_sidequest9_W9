...
// NEW: debug mode variables
let debugScreenVisible = false; // toggle debug panel visibility
let moonGravityEnabled = false; // toggle moon gravity on/off
let showHitboxes = false; // toggle collision box visibility
const normalGravity = GRAVITY; // store normal gravity value
const moonGravity = 3; // weaker gravity for moon mode
...
function keyPressed() {
  ...
  // NEW: debug key toggle logic
  if (key === "d" || key === "D") {
    debugScreenVisible = !debugScreenVisible; // toggle debug screen
    return false; // prevent this key press from affecting gameplay input
  }
  if (key === "g" || key === "G") {
    moonGravityEnabled = !moonGravityEnabled; // toggle moon gravity
    world.gravity.y = moonGravityEnabled ? moonGravity : normalGravity;
  }
  if (key === "h" || key === "H") {
    showHitboxes = !showHitboxes; // toggle hitbox visibility
  }
}
...
function draw() {
  ...
  // NEW: explicit movement keys so D is reserved for debug toggle
  const moveLeft = keyIsDown(LEFT_ARROW) || keyIsDown(65); // Left Arrow or A
  const moveRight = keyIsDown(RIGHT_ARROW); // Right Arrow only
  ...
  if (attacking) {
    ...
    player.ani = moveLeft || moveRight ? "run" : "idle";
  }
  ...
  // NEW: debug panel drawing
  if (debugScreenVisible) {
    drawDebugPanel();
  }

  // NEW: show hitboxes if enabled
  if (showHitboxes) {
    drawHitboxes();
  }
}

// NEW: debug panel drawing function
function drawDebugPanel() {
  // Save current drawing state to prevent affecting normal rendering
  push();
  ...
  let yPos = 15;
  text("DEBUG PANEL (D to toggle)", 15, yPos);
  yPos += 20;

  // Moon gravity status
  text(`Moon Gravity (G): ${moonGravityEnabled ? "ON" : "OFF"}`, 15, yPos);
  yPos += 15;

  // Hitboxes status
  text(`Hitboxes (H): ${showHitboxes ? "ON" : "OFF"}`, 15, yPos);
  yPos += 15;

  // Player coordinates
  text(`Player X: ${player.x.toFixed(1)}`, 15, yPos);
  yPos += 15;
  text(`Player Y: ${player.y.toFixed(1)}`, 15, yPos);
  yPos += 15;

  // FPS
  text(`FPS: ${frameRate().toFixed(1)}`, 15, yPos);

  // Restore previous drawing state
  pop();
}

// NEW: hitbox drawing function
